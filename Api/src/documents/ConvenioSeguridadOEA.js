...
/**
 * Genera el PDF del Convenio de Seguridad en la Cadena de Suministro.
 * @param {Object} params
 * @param {string} params.nombreRepresentante  - Nombre del representante del socio comercial
 * @param {string} params.nombreEmpresa        - Nombre de la empresa socia
 * @param {string} params.dia                  - Día de firma (ej. "14")
 * @param {string} params.mes                  - Mes de firma  (ej. "noviembre")
 * @param {string} params.anio                 - Año de firma  (ej. "2024")
 * @returns {PDFDocument}  Instancia del documento lista para pipe
 */
export function generarConvenioSeguridad({ nombreRepresentante, nombreEmpresa, dia = "14", mes = "noviembre", anio = "2024" }) {
	...
	// ── Firmas ───────────────────────────────────────────────────────────────
	const lineY = doc.y
	const colRight = doc.page.width / 2

	// Columna izquierda – Agencia aduanal
	doc
		.text("____________________________________________", 72, lineY, { width: W / 2, align: "left" })
		.moveDown(0.3)
		.text("A.A. CESAR AUGUSTO SAVIÑON RUELAS", 72, doc.y, { width: W / 2, align: "left" })
		.text("Saviñon Agencia Aduanal, SC", 72, doc.y, { width: W / 2, align: "left" })

	// Columna derecha – Socio comercial
	doc
		.text("_____________________________________________", colRight, lineY, { width: W / 2, align: "right" })
		.moveDown(0.3)
		.text("Representante Socio Comercial", colRight, doc.y, { width: W / 2, align: "right" })
